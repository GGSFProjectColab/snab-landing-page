...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      ...
      // Pro pattern: keep callback minimal, only DOM mutation. Supports both
      // `light`/`dark` classes (our globals uses :root=dark, .light, .dark)
      // instead of only toggling `dark` — avoids stale class combination.
      const rootEl = document.documentElement
      rootEl.classList.remove("light", "dark")
      rootEl.classList.add(newTheme ? "dark" : "light")
      if (isControlled) {
    ...
    // Fast-paths: reduced motion → instant, unsupported → instant.
    // This respects accessibility (no motion discomfort) and keeps the
    // callback cheap — no snapshot memory, no compositor work.
