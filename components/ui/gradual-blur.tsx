...
const DEFAULT_CONFIG: Required<Omit<GradualBlurProps, "hoverIntensity" | "onAnimationComplete" | "preset">> = {
  position: "bottom",
  strength: 2,
  height: "6rem",
  width: "",
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${capitalizedKey}`])
        v = config[`mobile${capitalizedKey}`] as string;
      else if (w <= 768 && config[`tablet${capitalizedKey}`])
        v = config[`tablet${capitalizedKey}`] as string;
      else if (w <= 1024 && config[`desktop${capitalizedKey}`])
        v = config[`desktop${capitalizedKey}`] as string;
      setValue(v);
    };
...
function GradualBlurInner(props: GradualBlurProps) {
  ...
  const config = useMemo(() => {
    ...
    return mergeConfigs(DEFAULT_CONFIG, presetConfig, props as Record<string, unknown>) as Required<GradualBlurProps>;
  }, [props]);

  const responsiveHeight = useResponsiveDimension(config.responsive, config as unknown as Record<string, unknown>, "height");
  const responsiveWidth = useResponsiveDimension(config.responsive, config as unknown as Record<string, unknown>, "width");
