...
function MapMarker({
  ...
  const marker = useMemo(() => {
    ...
    markerInstance
      .getElement()
      ?.addEventListener("click", (e) => callbacksRef.current.onClick?.(e));
    markerInstance.getElement()?.addEventListener("mouseenter", (e) => {
      callbacksRef.current.onMouseEnter?.(e);
    });
    markerInstance.getElement()?.addEventListener("mouseleave", (e) => {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        ...
        // Ignore cleanup races while MapLibre is tearing down.
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoaded, map]);
