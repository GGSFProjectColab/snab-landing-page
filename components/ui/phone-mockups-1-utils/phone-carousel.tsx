...
export function PhoneCarousel({
  ...
  return (
    <div className={cn("relative flex flex-col items-center justify-center max-h-full", className)}>
      ...
      <div className="mt-2.5 sm:mt-3 md:mt-3 flex items-center gap-2.5 sm:gap-3 md:gap-3 z-20 shrink-0">
        <Button
          variant="outline"
          size="icon"
          onClick={handlePrev}
          aria-label="Previous mobile screen"
          className="h-6 w-6 sm:h-6.5 sm:w-6.5 md:h-7 md:w-7 rounded-full border-dotted border-edge bg-background/80 hover:bg-muted text-foreground p-0"
        >
          <ChevronLeft className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-3.5 md:w-3.5" />
        </Button>
        ...
        <div className="flex items-center gap-1.5 sm:gap-1.5 md:gap-2">
          {images.map((_, idx) => (
            <button
              ...
              className={cn(
                "h-1.5 sm:h-1.5 md:h-2 rounded-full transition-all duration-300",
                idx === currentIndex
                  ? "w-4 sm:w-4.5 md:w-5 bg-teal"
                  : "w-1.5 sm:w-1.5 md:w-2 bg-muted-foreground/30 hover:bg-muted-foreground/60"
              )}
            />
          ))}
        </div>
        ...
        <Button
          ...
          className="h-6 w-6 sm:h-6.5 sm:w-6.5 md:h-7 md:w-7 rounded-full border-dotted border-edge bg-background/80 hover:bg-muted text-foreground p-0"
        >
          <ChevronRight className="h-3 w-3 sm:h-3.5 sm:w-3.5 md:h-3.5 md:w-3.5" />
        </Button>
      </div>
    </div>
  );
